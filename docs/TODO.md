...
- [~] Mobile: QR okuma, kısa kod girişi — kısa kod [x] (`PayCodeScreen`, 6 hane); QR kamera [~] native modül bekliyor (LESSONS.md RN native kaydı): `PayQrScreen`'de "QR okuyucu native kurulumla gelecek" placeholder + manuel kod girişi fallback (codeType=1), TODO yorumu ile
...
- [x] Mobile: transfer ana menü — `TransferMenuScreen`: Kartlarım Arası / Cep Numarasına / Kayıtlı Alıcı / QR Kod Alıcı / İşlem Geçmişi; **"Başka Karta" DEVRE DIŞI "Yakında" rozetiyle** (sözleşme boşluğu, üstteki [!] notu)
- [x] Mobile: Kartlar Arası (gönderen/alıcı/cüzdan/tutar/açıklama) — `TransferFormScreen`: gönderen/alıcı kart seçimi (bottom sheet), cüzdan (RESTORAN/MARKET), hızlı tutar 500/1000/2500/5000, tutar TAM TL (rakam dışı girişler engellenir; "500"→"500.00"), açıklama. NOT: receiver.type='card'+value=cardId gönderilir; backend 'card' türünü şimdilik reddediyor (üstteki sözleşme boşluğu) — backend kendi-kartı çözümlemesini açınca ekran değişmeden çalışır
- [~] Mobile: QR Kod Alıcı (okut→token) — resolve-qr çağrısı + maskeli alıcı → forma aktarım [x] (`TransferQrScreen`); QR kamera [~] native modül bekliyor: placeholder + manuel QR yükü girişi fallback (TODO yorumu)
